...
def count_skymap(pair, survey, inputs, footprint, sats, sat_cut=None, psi=None, n_trials=1):
    ...
    plt.savefig('realizations/{0}/skymaps_no-fixed_location/{0}_skymap_psi={1:0>3d}.png'.format(pair, psideg), bbox_inches='tight')
    plt.close()
...
if __name__ == '__main__':
    ...
    if args['psi'] is not None:
        count_skymap(args['pair'], survey, inputs, footprint, sats, sat_cut=cut, psi=args['psi'], n_trials=args['n_trials'])
    if args['rotations']:
